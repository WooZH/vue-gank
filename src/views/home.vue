<template>
  <div id="home">
    ...
    <div>
      ...
      <!--</ul>-->
      <!--每日试图-->
      <view-daily :viewData="homeData" v-if="!reqName">

      </view-daily>

      <!--iOS视图-->
      <view-iOS :viewData="viewData"
                v-if="reqName == 'iOS' || reqName == 'Android'|| reqName == 'all'|| reqName == '休息视频'|| reqName == '拓展资源'|| reqName == '瞎推荐'|| reqName == '前端'|| reqName == 'App'">


      </view-iOS>

      <!--福利视图-->
      <view-fl :viewData="viewData" v-if="reqName == '福利'">

      </view-fl>

      <!--<mugen-scroll :handler="loadMore" :should-handle="!isloadmore"></mugen-scroll>-->
      <mugen-scroll :handler="loadMore"></mugen-scroll>
      <!--<div v-if="!reqName">-->
      <div>
        <transition name="transX">

          <div class="toolbar" @click="clickDateBtn" v-show="isToolbar && reqName == ''">
            <p>{{selectedDate}}</p>
          </div>
        </transition>
        ...
        <transition name="up">
          <div class="random-button" @click="refresh" v-show="isToolbar">
            <span class="icon iconfont icon-guanbi" v-if=" reqName != ''"></span>
            <span class="icon iconfont icon-guanbi" v-if=" reqName == ''"></span>
          </div>
        </transition>

        <transition name="down">
          <div v-show="isToolbar" class="toTop-button iconfont icon-mp-top" @click="toTop()">
          </div>
        </transition>
      </div>
    </div>
...
<script>
  ...
  import viewFl from '../components/view-fl.vue'
  import viewDaily from '../components/view-daily.vue'
  import viewIOS from '../components/view-iOS.vue'
...
//          this.reqName = typeNo;
          this.reqUrl = this.sampleUrl + typeNo + '/' + this.itemCount + '/';
          this.viewData = {};
          this.pageNo = 1;
          this.getInfoType()
          this.reqName = typeNo;
...
//        显示加载
        this.isToolbar = false;
        let requestUrl = this.reqUrl + this.pageNo;
        this.isLoading = true;
        this.$http.get(requestUrl).then(function (resp) {
          if (!resp.error) {
            if(this.pageNo > 1){
              this.viewData.results.concat(resp.data.results);

              for(var i =0; i< resp.data.results.length; i++){
                this.viewData.results.push(
                  resp.data.results[i]
                );
              }
              console.log(this.viewData.results);
            }
            else{
